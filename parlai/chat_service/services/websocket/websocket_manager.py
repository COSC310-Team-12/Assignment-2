...
class WebsocketManager(ChatServiceManager):
    ...
    def observe_message(self, socket_id, message, beam_texts =None, quick_replies=None):
        """
        ...
        if beam_texts is not None:
            beam_texts = list(beam_texts)
        message = json.dumps(
            {'text': message.replace('\n', '<br />'), 'beam_texts': beam_texts, 'quick_replies': quick_replies}
        )
